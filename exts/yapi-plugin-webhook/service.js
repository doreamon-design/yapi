...
class syncUtils {
    ...
    constructor(ctx) {
        ...
        this.webhookModel = yapi.getInst(webhookModel);
    ...
    //初始化定时任务
    async init() {
        let allSyncJob = await this.webhookModel.listAll();
    ...
    async syncInterface(projectId, swaggerUrl, syncMode, uid, projectToken) {
        ...
        try {
            ...
            //删除数据库定时任务
            await this.webhookModel.delByProjectId(projectId);
            return;
        }
        //如果项目已经删除了
        if (!oldPorjectData) {
        ...
        let oldSyncJob = await this.webhookModel.getByProjectId(projectId);
        ...
        if (newSwaggerJsonData && oldSyncJob.old_swagger_content && oldSyncJob.old_swagger_content == md5(newSwaggerJsonData)) {
            ...
            await this.webhookModel.upById(projectId, oldSyncJob);
            return;
        }
        ...
        if (requestObj.body.errcode == 0) {
            ...
            await this.webhookModel.upById(oldSyncJob._id, oldSyncJob);
        }
        //记录日志
